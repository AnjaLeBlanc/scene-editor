...
public class AudioFile extends MediaObjects implements ControllerListener {
	...
	private PrerecordedAudioState state = PrerecordedAudioState.NO_FILE;
	
	public enum PrerecordedAudioState {
		NO_FILE, 
		AUDIO_STOPPED, 
		AUDIO_PLAYING
	}
...
//	private JarExtractor je = new JarExtractor(LiveAudio.class.getProtectionDomain().getCodeSource().getLocation().getFile());
	...
	public AudioFile() {
		super();
		this.spacehoderImage = new ImageIcon(je.getImageData("images/musiccd.png")).getImage();
	}
	
	public AudioFile(Node node){
		super();
		this.spacehoderImage = new ImageIcon(je.getImageData("images/musiccd.png")).getImage();
...
//			System.out.println("childnodes " + nl.item(i).toString());
	...
	public AudioFile(int x, int y) {
		super();
		System.out.println("new AudioFile");
	...
	public void start() {
		player.setMediaTime(new Time(this.seek));
		player.start();
		state = PrerecordedAudioState.AUDIO_PLAYING;
	}

	public void stop() {
		if( player != null ){
			player.stop();
			state = PrerecordedAudioState.AUDIO_STOPPED;
		}
	}
	
	public void setState(PrerecordedAudioState state) {
		this.state = state;
	}
	...
	public void startReceivingAudio(MediaLocator locator, long ssrc) throws IOException, NoPlayerException {

		System.out.println("start playing audio");
		this.ssrc  = ssrc;
		player = javax.media.Manager.createProcessor(locator);
...
//		player.start();
//		player.setMediaTime(new Time(120d));
		System.out.println("should hear something now");
		this.state = PrerecordedAudioState.AUDIO_STOPPED;
	...
	public PrerecordedAudioState getState() {
		return state;
	}

	public void setSeek(long timeOffset) {
		System.out.println("seek " + timeOffset);
		this.seek = timeOffset/1000;
		if (player != null){
			player.setMediaTime(new Time(seek));
		}
	}
