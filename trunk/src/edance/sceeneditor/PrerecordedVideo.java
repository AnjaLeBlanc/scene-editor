...
public class PrerecordedVideo extends MediaObjects {
	...
	// The renderer for the movie
	private RGBRenderer renderer = null;

	private PrerecordedVideoState state = PrerecordedVideoState.NO_VIDEO;
	...
	// ssrc of the current video;
	private long ssrc = 0;

	// transparency value for the video
	private int transparency = 0;

	public enum PrerecordedVideoState {
		NO_VIDEO, VIDEO_STOPPED, VIDEO_PLAYING
	}
	...
	public java.lang.Object clone() throws CloneNotSupportedException {
		PrerecordedVideo clone = (PrerecordedVideo) super.clone();
	...
	private double videoscale = 1.0f;

	private boolean visible = true;

//	private JarExtractor je = new JarExtractor(PrerecordedVideo.class.getProtectionDomain().getCodeSource().getLocation().getFile());
	private JarExtractor je = new JarExtractor("edance.jar");

	public PrerecordedVideo() {
		super();
		this.spacehoderImage = new ImageIcon(je.getImageData("images/tape30x16.png")).getImage();
	}
	
	public PrerecordedVideo(Node node){
		super();
		this.spacehoderImage = new ImageIcon(je.getImageData("images/tape30x16.png")).getImage();
		...
		if(nnm != null){
			for(int i = 0; i < nnm.getLength(); i++){
				...
				if (n2.getNodeName().trim().compareTo("SOURCE") == 0) {
					System.out.println("set source " + n2.getNodeValue());
				...
				} else if (n2.getNodeName().trim().compareTo("TIME_OFFSET") == 0) {
					this.seek =Long.valueOf(n2.getNodeValue());
				}
			}
		}
		...
		System.out.println("finished laoding prerecorded Video");
	}

	public PrerecordedVideo(int x, int y) {
		super();
	...
	public boolean setKey(int key, boolean ctrl) {
		...
		if ((key == 'D') || (key == 'd')) {
			setSize((int) size.getWidth() + woff, (int) size.getHeight() + hoff);
			returnvalue = true;
		}
	...
	public void paint(Graphics g) {
		...
		if (renderer == null) {
			...
			g.drawImage(this.spacehoderImage, (int) (this.positionX
					* MediaObjects.scaleToDesign + 1),
					(int) (this.positionY * MediaObjects.scaleToDesign),
	...
	public boolean selectpoint(int x, int y) {
		...
		if (x - x1 < BORDERSPACE && x - x1 > -BORDERSPACE && y - y0 < BORDERSPACE && y - y0 > -BORDERSPACE) {
			// System.out.println("select 3");
			select = POINTS.POINT2;
			return (true);
		}
		if (x - x0 < BORDERSPACE && x - x0 > -BORDERSPACE && y - y1 < BORDERSPACE && y - y1 > -BORDERSPACE) {
			// System.out.println("select 4");
			select = POINTS.POINT3;
			return (true);
		} else if (x - x1 < BORDERSPACE && x - x1 > -BORDERSPACE && y - y1 < BORDERSPACE && y - y1 > -BORDERSPACE) {
	...
	public void movepoint(int x, int y) {
		...
		if (select == POINTS.POINT2 || select == POINTS.POINT2) {
			currSize[1] -= y;
			this.positionY += y;
		}
		changePreviewRenderer();
	...
	public void stop() {
		...
		if (renderer != null) {
			...
			renderer.stop();
//			dataSource.stop();
			state = PrerecordedVideoState.VIDEO_STOPPED;
		}
	}

	public void setState(PrerecordedVideoState state) {
			this.state = state;
	}
	...
	public PrerecordedVideoState getState() {
		return state;
	}
	...
	public void setVideoscale(double videoscale) {
	...
	public Element save(Document xmldoc) {
		...
		e.setAttributeNS(null, "NAME", this.getClass().getName());
		...
		e.setAttribute("TIME_OFFSET", String.valueOf(this.seek));
		return e;
	}
}
